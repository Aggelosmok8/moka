...
export default function MatchAnalysisPage() {
  ...
  // Available odds for the LION pick, sorted best (highest) to worst.
  ...
  return (
    <Shell>
      ...
      {/* LION LEAN — aligned to the model's expected outcome */}
      {!isLive && (
      <Card testId="moka-pick">
        <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 mb-1">LION Lean</div>
        <div className="flex items-end justify-between gap-3 flex-wrap">
          <div>
            <div className="font-display font-black text-2xl text-[#39FF14]">{value.possibleOutcome || value.pickName}</div>
            <div className="text-sm text-zinc-400 mt-0.5">LION's expected outcome from recent form &amp; scoring</div>
          </div>
        </div>
      ...
      <Card testId="ai-analysis">
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="font-display font-bold uppercase tracking-tight text-sm text-white">LION Analysis</h3>
          </div>
          <button
            type="button"
        ...
        {aiLoading ? (
          <div className="flex items-center gap-2 text-sm text-zinc-400 py-4"><Loader2 className="w-4 h-4 animate-spin" /> Writing the LION analysis… <span className="text-zinc-600">first view takes a few seconds, then it's instant</span></div>
        ) : ai && ai.analysis ? (
          ...
          <div className="text-sm text-zinc-500 py-2">AI analysis is temporarily unavailable — the LION model prediction is shown below.</div>
        )}
      ...
      {/* WHY LION LIKES IT */}
      {!isLive && (
      <Card title="Why LION likes it" testId="why-moka">
      ...
      {/* LION PREDICTION — basic, visible to all */}
