...
if (!BACKEND_URL && import.meta.env.PROD) {
  ...
  console.warn(
    "[LION] VITE_BACKEND_URL is not set — API calls fall back to a relative '/api' path and will fail in production. Set VITE_BACKEND_URL in your Vercel project environment variables."
  );
}
...
export const fetchTeamRecent      = (teamId, last = 6) => api.get(`/teams/${teamId}/recent`, { params: { last } }).then(r => r.data.matches || []);
